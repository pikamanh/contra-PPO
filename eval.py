...
def run_episode(model, vec_env, screen, font, font_bold, clock, fps: int, record: bool = False):
    ...
    while not done:
        ...
        hud = [
            ...
            ("boss defeated", str(env_info.get('boss_defeated', '?')),
                                                               (255, 120, 255)),
            ("boss state", f"{env_info.get('boss_defeated_state', '?')}",
                                                               (255, 160, 255)),
            ("stage over", str(env_info.get('stage_over', '?')),
                                                               (120, 255, 180)),
            ("won",       str(env_info.get('won', '?')),      (80, 255, 120)),
